"""Native cognis-connect emit for vendorvet — forward findings to any platform.

Maps vendorvet's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    vendorvet ... --format json | vendorvet-emit --to stix
    vendorvet-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['severity'] = rec.get('risk_tier', 'info')
