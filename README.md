...
[![PyPI](https://img.shields.io/pypi/v/cognis-vendorvet.svg)](https://pypi.org/project/cognis-vendorvet/)
[![CI](https://github.com/cognis-digital/vendorvet/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/vendorvet/actions)
...
VENDORVET exists for one job — third-party / vendor risk questionnaires with sbom cross-ref — and does it without a SaaS bill or heavyweight setup.
...
```bash
vendorvet --version
vendorvet scan .                      # scan the current project
vendorvet scan . --format json
vendorvet scan . --fail-on high       # non-zero exit for CI gates
vendorvet mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **OneTrust TPRM**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`vendorvet` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
